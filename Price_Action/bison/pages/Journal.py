            ...
            for index, row in df.iterrows():
                with st.expander(f"{row.get('Stock Name', 'N/A')} {row.get('Date', 'N/A')} "):
                    ...
                    if 'Image' in df.columns and row['Image']:
                        ...
                        # Ensure image_path is a string and not NaN
                        if pd.notnull(image_path) and isinstance(image_path, str):
                            # Check if the file exists and is an image
                            if os.path.exists(image_path) and Image.open(image_path).format:
                                st.image(image_path, caption='Uploaded Image', use_column_width=True)
                            else:
                                st.warning(f"Image not found or not an image file: {image_path}")
                        else:
                            st.warning("Image path is not valid.")
